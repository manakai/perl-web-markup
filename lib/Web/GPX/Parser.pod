...
I<$parsed>, which has zero or more of folloging key/value pairs:
...
=item $url = $parsed->{page_url}

The canonicalized absolute URL of the Web page associated with the GPX
document, if any.

=item $arrayref_of_routes = $parsed->{routes}

The array reference of zero or more route data structures,
representing routes in the GPX document.  There is always a C<routes>
...
=item $arrayref_of_tracks = $parsed->{tracks}

The array reference of zero or more track data structures,
representing tracks in the GPX document.  There is always a C<tracks>
member in the data strucuture.
...
=item $arrayref_of_points = $parsed->{waypoints}

The array reference of zero or more point data structures,
representing waypoints in the GPX document.  There is always a
C<waypoints> member in the data strucuture.
...
=item $url = $route->{page_url}

The canonicalized absolute URL of the Web page associated with the
route, if any.

=item $arrayref_of_points = $route->{points}

The array reference of zero or more point data structures,
representing the points in the route.  There is always a C<points>
member in the data strucuture.
...
=item $url = $track->{page_url}

The canonicalized absolute URL of the Web page associated with the
route, if any.

=item $arrayref_of_segments = $track->{segments}

The array reference of zero or more track segment data structures,
representing the segments in the track.  There is always a C<segments>
member in the data strucuture.
...
which has zero or more of folloging key/value pair:

=over 4

=item $arrayref_of_points = $segment->{points}
...
has zero or more of folloging key/value pairs:
