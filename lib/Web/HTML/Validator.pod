...
=item $code = $val->onsubdoc

=item $val->onsubdoc ($code)

Get or set the subdocument validation handler hooks.  Some nested
constructs in the document, e.g. embedded JavaScript codes, regular
expressions, and C<iframe> C<srcdoc> documents, are considered as
"subdocument".  The validation itself does not check the conformance
of the subdocument, but it instead invoke this callback such that
appropriate validator modules can be dispatched.  The callback is not
...
See <http://suika.suikawiki.org/gate/2005/sw/Whatpm%20Error%20Types>.
...
Errors and warnings are reported through the C<onerror> handler.  Any
subdocument is reported through the C<onsubdoc> handler.
