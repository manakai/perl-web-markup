...
character encoding cannot be determined by other means) and to expand
the C<isindex> start tag (which is obsolete).

If C<undef> is specified (or the C<locale_tag> method is not
explicitly invoked at all), the default is "none", which results in
the C<windows-1252> character encoding and English text defaults.
...
the |noscript| element's content is not parsed (This is how browsers
