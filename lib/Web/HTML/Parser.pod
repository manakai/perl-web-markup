...
parsing algorithm defined by the HTML Standard and implemented by Web
browsers.

This module is intended to be a component of Web user agents, such as
browsers, data mining tools, validators, and so on.  However, such
applications might not want to use this module directly.  Rather, this
module should be used indirectly through more abstract or
domain-specific modules, such as DOM implementations, scrapers, or
feed processors, which can provide more convinient APIs.

The perl-web-dom package contains an implementation of the standard
DOM APIs in pure Perl, which implements HTML parsing API including
C<inner_html> and C<DOMParser> using this module.  See L<Web::DOM>
...
Parse a character string as HTML.  The first argument must be the
character string (i.e. a latin1 or utf8 string) to parse.  The second
argument must be a DOM C<Document> object.  The C<Document> is mutated
...
L</SPECIFYING ENCODING>).  The second argument must be the byte string
to parse.  The third argument must be a DOM C<Document> object.  The
C<Document> is mutated during the process of parsing.
...
argument must be the character string (i.e. a latin1 or utf8 string)
to parse.  The second argument must be an C<Element> object used as
the context, or C<undef> if there is no context.  The third argument
must be an empty C<Document> object used in the process of parsing.
Note that the C<Document>'s children list is not affected by the
parsing.  The method returns an C<HTMLCollection> object containing
the result of the parsing (zero or more C<Node> objects).
