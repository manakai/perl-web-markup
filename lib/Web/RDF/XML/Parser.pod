...
containing one or more of followings: C<uri> (URL of a resource),
C<bnodeid> (graph-local ID of a blank node), C<value> (lexical form of
a literal), C<parent_node> (parent node of DOM representation of an
XML literal), C<language> (language tag of a literal), and C<datatype>
...
applying the following steps to the corresponding hash value /value/:
  ...
  1. If /value/ has non-|undef| |uri|, return URL (or "IRI" in RDF
     1.1) |uri| and abort these steps.
  ...
  3. If /value/ has |parent_node|, return a literal whose lexical form
     ...
     "IRI" in RDF 1.1) is /value/'s |datatype| (which is not |undef|)
     and abort these steps.

  4. If /value/ has non-|undef| |language|, return a literal whose
     lexical form is /value/'s |value|, language tag is /language/'s
     value, and datatype URL (or "IRI" in RDF 1.1) is
     |http://www.w3.org/1999/02/22-rdf-syntax-ns#langString| and abort
     these steps.

  5. Otherwise, return a literal whose lexical form is /value/'s value
     (which is not |undef|) and datatype URL (or "IRI" in RDF 1.1) is
     /value/'s |datatype|, if not |undef|, or
