...
(or C<undef> to unset).  The code reference would be invoked with an
...
exception.  Note that the null namespace cannot be represented.
...
The parser MUST resolve any namespace prefix in the input string at
the same time as the parsing.  If the prefix cannot be resolved, it
MUST throw a namespace error and abort the parsing process.
