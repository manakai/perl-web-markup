...
C<concat()>, C<contains()>, C<id()>, C<lang()>, C<string()>,
C<substring-before()>, C<substring-after()>, and C<translate()> and
the C<=> and C<!=> operators.  In other words, they are implemented in
the same way as XPath implementations strictly conforming to the XPath
1.0 specification and their result can be different from those in Web
browsers.
...
DOM Standard's definition for unique identifier (ID) [DOM] MUST be
used for the C<id()> function.
