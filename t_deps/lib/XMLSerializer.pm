      ...
      if ($srcnt == 1) { # ELEMENT_NODE
        ...
        XA: for my $attr (@nsattrs) {
          ...
          if ($lname eq 'xmlns') {
            ...
            $attrr{xmlns} = [$xescape->($attr->value)];
          } else {
            if (length $attrval) {
              $ns->{$lname} = $attrval;
            } else {
            ...
            $attrr{'xmlns:'.$lname} = [$xescape->($attr->value)];
          }
        } # XA
        ...
        for my $attr (@lattrs) {
          ...
          $attrr{$attr->local_name} = [$xescape->($attr->value)];
        }
        ...
        for my $attr (@gattrs) {
          ...
          $attrr{$atpfx.':'.$attr->local_name} = [$xescape->($attr->value)];
        }
